...
module Necropsy
  class AstScanner
    # Owns framework and Ruby DSL semantics while the scanner coordinates
    # traversal and state. The scanner is passed as a narrow context so the
    # existing graph-building behavior remains unchanged.
    class DslRules
      ...
      %i[
        add_definition add_generated_methods arguments callback_names class_record constant_candidates constant_name
        convention_ancestors custom_validator_names delegated_method_name handled_call keyword_keys keyword_value
        literal_argument literal_name? literal_value method_kind_and_separator project record_delegate_target
        record_delegated_message
        entrypoint_hints record_semantic_blocker symbol_arguments visit visit_synthetic_body
      ].each do |method_name|
        define_method(method_name) do |*args, **kwargs, &block|
          @scanner.send(method_name, *args, **kwargs, &block)
        end
      end
